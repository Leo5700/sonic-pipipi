...
blocksize = 44100//1  # величина блока, забираемого из потока
...
#    d = np.argmin(np.abs(c), 1)  # индексы ближайших a к b
#    f = np.zeros_like(b)
...
#        block = np.random.rand(blocksize)  ### симулируем поток realtime
        ...
        with sd.InputStream(device=1, callback=callback, blocksize=blocksize):
            sd.sleep(int(blocksize / bitrate * 1000 + 200))
